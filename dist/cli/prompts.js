...
export const menuPrincipal = () => inquirer.prompt([{
...
export const promptLivro = () => inquirer.prompt([
    ...
    { name: 'ano', message: 'Ano:', validate: (v) => isNaN(Number(v)) ? 'Informe número' : true, filter: (v) => Number(v) }
]);
...
export const promptLivroPatch = () => inquirer.prompt([
    ...
    { name: 'isbn', message: 'Novo ISBN (vazio para não alterar):' },
    { name: 'ano', message: 'Novo ano (vazio para não alterar):', filter: (v) => v ? Number(v) : undefined }
]);
...
export const promptEmprestimo = () => inquirer.prompt([
    ...
    { name: 'dias', message: 'Dias de empréstimo (padrão 7):', default: 7, filter: (v) => Number(v) }
]);
